<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="java.util.*, com.kosa.mvc2.board.*, com.kosa.mvc2.common.*" %>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
   ...
   <colgroup>
    ...
    %>
      <tr>
        ...
        <td><a href="<%=url%>"><%=dto.getB_b_title() %></a></td>
        <td><%=dto.getB_b_writer() %></td>
        <td><%=dto.getB_b_wdate() %></td>
        <td><%=dto.getB_b_hit() %></td>
      </tr>
      <%
    }
      %>
...
<script>
	...
	window.onload=()=>{
		...
		let b_m_userid = '<%=session.getAttribute("b_m_userid")%>';
		console.log(b_m_userid);
		if(b_m_userid=="null"||b_m_userid==""){
			alert("로그인이 필요합니다.");
