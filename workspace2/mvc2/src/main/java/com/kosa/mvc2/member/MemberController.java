...
public class MemberController extends HttpServlet {
	...
	void doIdCheck(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String m_userid = request.getParameter("b_m_userid");
	...
	void doSave(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String b_m_userid = request.getParameter("b_m_userid");
		String b_m_name = request.getParameter("b_m_name");
		String b_m_password = request.getParameter("b_m_password");
		String b_m_email = request.getParameter("b_m_email");
		String b_m_phone = request.getParameter("b_m_phone");
		
		// dto에 값 저장
		dto.setB_m_userid(b_m_userid);
		dto.setB_m_name(b_m_name);
		dto.setB_m_password(b_m_password);
		dto.setB_m_email(b_m_email);
		dto.setB_m_phone(b_m_phone);
	...
	void doLoginProc(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String b_m_password = request.getParameter("b_m_password");
		MemberDAO dao = new MemberDAO();
		MemberDTO dto = dao.login(b_m_userid, b_m_password);
		JSONObject obj = new JSONObject();
		if(dto.getB_m_userid().equals("")) {
			// 로그인 실패시
			obj.put("result", "fail");
		}else {
			...
			session.setAttribute("b_m_userid", dto.getB_m_userid());
			session.setAttribute("b_m_name", dto.getB_m_name());
			session.setAttribute("b_m_password", dto.getB_m_password());
	...
	void doFindIdProc(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String b_m_email = request.getParameter("b_m_email");
		MemberDAO dao = new MemberDAO();
		String id = dao.findId(b_m_name, b_m_email);
		JSONObject obj = new JSONObject();
		if(id.equals("")) {
			obj.put("result", "fail");
		}else {
	...
	void doFindPasswordProc(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String b_m_email = request.getParameter("b_m_email");
		MemberDAO dao = new MemberDAO();
		String password = dao.findPassword(b_m_userid, b_m_email);
		JSONObject obj = new JSONObject();
		if(password.equals("")) {
			// 비밀번호를 못 찾으면
			obj.put("result", "fail");
